...
\begin{document}
	...
	Per poter impostare correttamente e in modo sicuro i cookie è necessario indicare il dominio del sito web, per farlo si deve modificare il file "config.php" presente nella root, indicando il dominio sul quale andrà utilizzato il sito.
	...
	\chapter{Validazione e test}
		...
		\section{Validazione con test automatici}
			...
			\end{figure}
			Dopo le correzioni sono ancora presente delle problematiche in locale, ma sono dettate dalla configurazione attuale di XAMPP configurato per essere utilizzato in un ambiente di sviluppo e non di produzione.
		...
		\section{Test con diverse impostazioni utente}
			...
			\`{E} stata presa in considerazione anche la possibilità che un utente abbia bloccato, oppure non abbia la possibilita se pur remota di non poter eseguire Javascript, nonstante questo il sito rimane comunque egualemente navigabile in tutte le sue parti, senza far trasparire in modo palese la mancanza di Javascript.\\
	...
	\begin{itemize}
		...
		\item Stile login e form lato admin
		\item Controllo javascript form  lato admin
		\item Parte dello sticky header
		\item Test su MacOS High Sierra (v10.13.2) e ios 11.2.5
		\item
		\item
	\end{itemize}
